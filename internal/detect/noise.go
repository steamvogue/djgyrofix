...
type NoiseProfile struct {
	...
	// NoisyDPS is the level the fractions were measured against: half the
	// effective detection floor, so the test scales with --floor-dps and
	// --sensitivity instead of hardcoding an absolute number.
	NoisyDPS float64 `json:"noisy_dps"`
}

// noiseProfile summarises the baseline curve produced by thresholdCurve.
//
// The comparison level is half the effective floor rather than an absolute
// constant. FloorDPS is the tool's own statement of "a residual below this is
// not worth correcting"; a typical residual within a factor of two of it means
// the clip is sitting near the defect level everywhere, which is a property of
// the airframe rather than of the recorded metadata.
func noiseProfile(baselines []float64, params Params, binSeconds float64) NoiseProfile {
	profile := NoiseProfile{NoisyDPS: effectiveFloor(params) / 2}
	if len(baselines) == 0 {
		return profile
	}
